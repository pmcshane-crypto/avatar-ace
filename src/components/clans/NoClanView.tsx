...
import { Users, Sparkles, ArrowRight, Search, Hash, Plus } from 'lucide-react';
...
export function NoClanView({ userId, onJoinClan }: NoClanViewProps) {
  ...
  const [activeView, setActiveView] = useState<'main' | 'code' | 'browse' | 'create'>('main');
  ...
  const loadPublicClans = async () => {
    const { data } = await supabase.from('clans').select('*').eq('is_public', true).limit(10);
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      <div className="absolute inset-0 bg-cover bg-center bg-no-repeat" style={{ backgroundImage: `url(${clanBeachBg})` }} />
        ...
        <div className="max-w-lg mx-auto space-y-8">
          ...
          {activeView === 'main' && (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-3">
              {/* Join by code */}
              <Card className="p-6 cursor-pointer hover:border-primary/50 transition-all group" onClick={() => setActiveView('code')}>
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground">Have a clan code? Enter it here</p>
                  </div>
              ...
              {/* Browse clans */}
              <Card className="p-6 cursor-pointer hover:border-primary/50 transition-all group" onClick={loadPublicClans}>
                <div className="flex items-center gap-4">
                  <div className="bg-accent/20 rounded-full p-3 group-hover:bg-accent/30 transition-colors">
                    <Users className="w-6 h-6 text-accent" />
                  </div>
                  <div className="flex-1">
                    <h3 className="font-semibold text-foreground">Browse Clans</h3>
              ...
              {/* Create clan */}
              <Card className="p-6 cursor-pointer hover:border-primary/50 transition-all group" onClick={() => setActiveView('create')}>
                <div className="flex items-center gap-4">
                  <div className="bg-primary/20 rounded-full p-3 group-hover:bg-primary/30 transition-colors">
          ...
          {activeView === 'code' && (
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
              <Card className="p-6 space-y-4">
                <Button variant="ghost" size="sm" onClick={() => setActiveView('main')} className="mb-2">← Back</Button>
                  ...
                  <Card key={clan.id} className="p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground line-clamp-1">{clan.description || 'No description'}</p>
                      ...
                      <Button size="sm" onClick={() => handleJoinClan(clan.id)} disabled={isLoading}>Join</Button>
                    </div>
                  </Card>
                ))}
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
              <Card className="p-6 space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input placeholder="Enter clan name" value={newClan.name} onChange={(e) => setNewClan({ ...newClan, name: e.target.value })} maxLength={30} />
                </div>
                <div className="space-y-2">
                  <Label>Description (optional)</Label>
                  <Textarea placeholder="What's your clan about?" value={newClan.description} onChange={(e) => setNewClan({ ...newClan, description: e.target.value })} maxLength={200} />
                </div>
                <div className="space-y-2">
                  <Label>Icon</Label>
                  <div className="flex flex-wrap gap-2">
                    {emojiOptions.map(emoji => (
                      <button key={emoji} onClick={() => setNewClan({ ...newClan, icon_emoji: emoji })}
                        className={`w-10 h-10 rounded-lg text-xl flex items-center justify-center transition-all ${newClan.icon_emoji === emoji ? 'bg-primary/30 border-2 border-primary' : 'bg-muted/30 hover:bg-muted/50'}`}>
                        {emoji}
                      </button>
                    ))}
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input type="number" value={newClan.daily_goal_minutes} onChange={(e) => setNewClan({ ...newClan, daily_goal_minutes: parseInt(e.target.value) || 120 })} min={30} max={480} />
                </div>
